    ...
    .then((data) => {
        ...
        api_data.forEach(dataAtTimestamp => {
            ...
            for (let i = 0; i < Object.keys(coinsIdentifier).length; i++) {
                ...
                coinObj.percent_change_1h =selectedCoin.quote.USD.percent_change_1h;
...
function pleaseResolveEverything() {
    ...
    for (let currency_index = 0; currency_index < num_of_currencies; currency_index++) {
        ...
        let old_val = coinmarketTop10[0][currency_index].price;

        for (let date_index = 1; date_index < num_of_dates; date_index++) {
            
            let new_val = coinmarketTop10[date_index][currency_index].price;
            let per_cha = (new_val / old_val) - 1;
            coinmarketTop10[date_index][currency_index].percent_change_1h = per_cha*100;
...
function draw() {
    ...
    if (coinmarketTop10.length > 0 && redditPosts['bitcoin'].length > 0) {
        ...
        for (let i = fi; i < coinmarketTop10.length; i++) {
        ...
        console.log('max change', max_change);
        ...
        for (let currency_index = 0; currency_index < num_of_currencies; currency_index++) {
            ...
            //Bars
            let last_date = coinmarketTop10[fi][currency_index].date;
            for (let date_index = fi; date_index < num_of_dates; date_index++) {
